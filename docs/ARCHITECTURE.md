...
flowchart TD
    ...
    API --> Guardrails[Apply Guardrails]
    Guardrails --> PII[PII Detection]
    PII --> Topic[Topic Filter]

    Topic -->|Blocked| Reject[Return Rejection]
    Topic -->|Allowed| Tools[Execute Tools]

    Tools --> Template[Template Retrieval]
    Tools --> Property[Property Lookup]
    Tools --> Reservation[Reservation Lookup]

    Template --> Generate[Generate Response]
    Property --> Generate
    Reservation --> Generate

    Generate -->|High Match| Direct[Direct Template]
    Generate -->|Medium Match| Refine[Template + LLM]
    Generate -->|Low Match| Custom[Custom LLM]

    Direct --> Response[Return Response]
    Refine --> Response
    Custom --> Response
    Reject --> Response
