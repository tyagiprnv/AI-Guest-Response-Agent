...
The AI Guest Response Agent uses a comprehensive evaluation framework based on the **LLM-as-Judge** pattern. This approach uses a capable LLM (GPT-4o-mini) to evaluate the quality of responses generated by the agent across multiple dimensions.
...
| `--model` | gpt-4o-mini | OpenAI model for judging |
...
- **Cost**: Should be < $0.01 per request on average
- **Template Match Rate**: Should be 70-80% for cost optimization
...
- **Cons**: Less nuanced than GPT-4

**Alternative**: GPT-4o
- **Pros**: Higher quality judgments
- **Cons**: More expensive (~10x cost)
...
**Solutions**:
- Use cheaper judge model (gpt-4o-mini vs gpt-4o)
